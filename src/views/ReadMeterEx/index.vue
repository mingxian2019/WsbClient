...
export default {
  ...
  computed: {
    ...
    list () {
      ...
      if (filterName) {
        ...
        let rest = XEUtils.searchTree(this.readExData, item => searchProps.some(key => XEUtils.toString(item[key]).toLowerCase().indexOf(filterName) > -1), options)
  ...
  mounted () {
    ...
    this.dateFrom = new Date(new Date(new Date().getTime() - 86400000).toDateString())
  ...
  methods: {
    initTable () {
      ...
      let initialData = spliceData(encodeData.length, Config.WsCmd.exLogGet, encodeData)
        ...
        if (serverData.flag === 1) {
          ...
          for (let i = 0; i < aMsg.Vec.length; i++) {
            this.readExData.push({
              ExTime: new Date(aMsg.Vec[i].ExTime * 1000).Format('yyyy-MM-dd HH:mm'),

  ...
  <div class="meter hw100">
    <vxe-toolbar :data="meterData"  setting>
      <template v-slot:buttons>
        ...
        <el-button @click="$refs.showAddMeter.show()"  size="small"  icon="el-icon-circle-plus-outline">新增</el-button>
    ...
    <vxe-table
      ...
      <vxe-table-column field="Addr" title="表地址" width="180" tree-node sortable>
    ...
    <el-popover
      ...
      <el-button  slot="reference" class="btn-fun group" size="small"  icon="el-icon-document-copy">分组</el-button>
    </el-popover>
...
export default {
  data () {
    return {
      ...
      loading: false,
      visible: false,
      protoMeterOfAreaOrTerminalIn: protoRoot.lookup('WsMsg.MeterOfAreaTerminalIn'),
      protoMeterOfAreaOrTerminalMsgOut: protoRoot.lookup('WsMsg.MeterOfAreaTerminalOut'),
      ...
      customColumns: [],
      origin: [],
      meterTypeFilters: [{ label: '电表', value: '电表' }, { label: '水表', value: '水表' }],
      radio: '',
      selectedMeters: [],
      currentMeter: null,
      online: true,
  ...
  computed: {
    list () {
      ...
      if (filterName) {
        ...
        let rest = XEUtils.searchTree(this.meterData, item => searchProps.some(key => XEUtils.toString(item[key]).toLowerCase().indexOf(filterName) > -1), options)
  ...
  methods: {
    ...
    initTable: function () {
      ...
      if (store.state.currentAreaTerNode.isTer) {
        ...
        meterOfAreaOrTerminalIn.TerminalAddr = store.state.currentAreaTerNode.addr
      } else {
        meterOfAreaOrTerminalIn.AreaId = store.state.currentAreaTerNode.id
        meterOfAreaOrTerminalIn.TerminalAddr = -1
      }

      let createProto = this.protoMeterOfAreaOrTerminalIn.create(meterOfAreaOrTerminalIn)
      let encodeData = this.protoMeterOfAreaOrTerminalIn.encode(createProto).finish()
      let initialData = spliceData(encodeData.length, Config.WsCmd.meterOfAreaTerminal, encodeData)
      sendSock(initialData, Config.WsCmd.meterOfAreaTerminal, serverData => {
        const aMsg = this.protoMeterOfAreaOrTerminalMsgOut.decode(serverData.data)
        if (serverData.flag === 1) {
          ...
          for (let i = 0; i < aMsg.Vec.length; i++) {
            this.meterData.push({
              ...
              JzqAddr: (aMsg.Vec[i].Protocol === 1) ? (parseInt(aMsg.Vec[i].VirAddr / 1024)).toString(16)
    ...
    delMeter () {
      ...
      for (let i = 0; i < this.selectedMeters.length; i++) {
        ...
        meters.VirAddrs.push(parseInt(addr * 1024) + parseInt(this.selectedMeters[i].SeqId))
      }
      let createProto = this.protoMeterDelIn.create(meters)
      let encodeData = this.protoMeterDelIn.encode(createProto).finish()
      let initialData = spliceData(encodeData.length, Config.WsCmd.meterDel, encodeData)
    ...
    handlePageChange (val) {
      const INTERCEPT = this.origin.slice((this.pageSize * (val - 1)), (val * this.pageSize))
    ...
    downloadTask () {
      ...
      for (var i = 0; i < meters.length; i++) {
        ...
        dataUnit.setUint8((27 * i) + 11,
          meters[i].Protocol === '3761' ? 30 : meters[i].Protocol === '97' ? 1 : 2, true)
        dataUnit.setUint32((27 * i) + 12, parseInt(meters[i].Addr, 16), true)
        dataUnit.setUint16((27 * i) + 16, parseInt(meters[i].Addr, 16) / 4294967296, true)

<template>
  ...
  <div class="payLogEx">
    ...
    <div class="table-box" ref="tableBox">
      ...
      >
        <vxe-table-column field="TradeNo" title="交易单号" min-width="120" show-overFlow>
          <template v-slot="{ row }">
        ...
        <vxe-table-column field="UserName" title="用户表名" min-width="120" show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.UserName"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="Mobile" title="手机号" min-width="120" show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.Mobile"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="PrePayType" title="预付费类型" min-width="120" show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.PrePayType"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="PayTime" title="收费时间" min-width="120" show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.PayTime"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="PayWay" title="收费方式" min-width="120" show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.PayWay"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="Fee" title="金额" min-width="120" show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.Fee"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="Cause" title="款项" min-width="120" show-overFlow>
          <template v-slot="{ row }">
            <span v-html="row.Cause"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="Receiver" title="收款员" min-width="120" show-overFlow>
          <template v-slot="{ row }">
            <span v-html="row.Receiver"></span>
          </template>
        </vxe-table-column>

        <vxe-table-column field="ActRet" title="执行失败次数" min-width="120" show-overFlow>
          <template v-slot="{ row }">
            <span v-html="row.ActRet"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="ActTime" title="最近执行时间" min-width="120" show-overFlow>
          <template v-slot="{ row }">
  ...
  methods: {
        ...
        if (serverData.flag === 1) {
          ...
          for (let i = 0; i < aMsg.Vec.length; i++) {
            this.meters.push({
              ...
              PayTime: new Date(aMsg.Vec[i].PayTime * 1000).Format(
                "yyyy-MM-dd HH:mm:ss"
              ),
              ...
              ActTime: new Date(aMsg.Vec[i].ActTime * 1000).Format(
                "yyyy-MM-dd HH:mm:ss"
              ),
    ...
    charge() {
      ...
      if (prePayType === 1) {
        ...
        let price = store.getters.getFeePriceByUserType(
          this.currentMeter.UserType
        );
        if (price < 0) {
      ...
      let initialData = spliceData(
        encodeData.length,
        Config.WsCmd.payExRetry,
        encodeData
      );
      sendSock(initialData, Config.WsCmd.payExRetry, serverData => {
        if (serverData.flag >= 0) {
          let index = this.meters.findIndex(
            meter => meter.TradeNo === this.currentMeter.TradeNo
          );

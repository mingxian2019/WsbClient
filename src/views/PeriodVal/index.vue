...
<!-- 时段用量查询 -->
  <div class='periodVal hw100'>
    ...
    <vxe-table
      ...
      <vxe-table-column field="Val0" title="起始时间表底">
        <template v-slot="{ row }">
          <span v-html="row.Val0"></span>
        </template>
      </vxe-table-column>
      <vxe-table-column field="Val1" title="结束时间表底">
        <template v-slot="{ row }">
          <span v-html="row.Val0"></span>
        </template>
      </vxe-table-column>
      <vxe-table-column field="Consume" title="用量">
        <template v-slot="{ row }">
...
export default {
  ...
  computed: {
    ...
    list () {
      ...
      if (filterName) {
        ...
        let rest = XEUtils.searchTree(this.periodValData, item => searchProps.some(key => XEUtils.toString(item[key]).toLowerCase().indexOf(filterName) > -1), options)
  ...
  methods: {
    ...
    initTable () {
      ...
      periodValQueryIn.EndTime = (Date.parse(this.dateEnd) + 86400000) / 1000 - 1
      ...
      let initialData = spliceData(encodeData.length, Config.WsCmd.periodValQuery, encodeData)
        ...
        if (serverData.flag === 1) {
          ...
          for (let i = 0; i < aMsg.Vec.length; i++) {
            this.periodValData.push({
              ...
              Val0: aMsg.Vec[i].Val0 === 4294967295 ? '--' : (aMsg.Vec[i].Val0 / 100).toFixed(2),
              Val1: aMsg.Vec[i].Val1 === 4294967295 ? '--' : (aMsg.Vec[i].Val1 / 100).toFixed(2),
              Consume: ((aMsg.Vec[i].Val0 !== 4294967295) && (aMsg.Vec[i].Val1 !== 4294967295)) ? (parseInt(val1 - val0) / 100).toFixed(2) : '无效'
            })
          }

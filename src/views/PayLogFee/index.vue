<template>
  ...
  <div class="payLogFee">
    ...
    <div class="table-box" ref="tableBox">
      ...
      >
        <vxe-table-column field="Addr" title="表地址" min-width="120px" tree-node show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.Addr"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="UserName" title="用户表名" min-width="120px"  show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.UserName"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="PrePayType" title="预付费类型"  min-width="120px" show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.PrePayType"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="PayTime" title="收费时间" min-width="120px"  show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.PayTime"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="PayWay" title="收费方式" min-width="120px"  show-overflow>
          <template v-slot="{ row }">
            <span v-html="row.PayWay"></span>
          </template>
        </vxe-table-column>
        <vxe-table-column field="Fee" title="金额"  min-width="120px" show-overflow>
          <template v-slot="{ row }">
